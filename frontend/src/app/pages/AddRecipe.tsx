...
import { createRecipeWithImage } from '../api';
...
export function AddRecipe() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!imageFile) {
      setIsSubmitting(false);
      setError('Пожалуйста, загрузите изображение блюда.');
      return;
    }
    ...
    try {
      await createRecipeWithImage(token, multipart);
      navigate('/admin');
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
      <div className="mb-10 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-[#1f2937]">Новый рецепт</h1>
          <p className="text-[#1f2937]/60 mt-2 text-lg">Заполните детали для публикации нового шедевра.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="md:col-span-2 space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Например: Идеальный авокадо тост"
                />
              </div>
            ...
            <div className="md:col-span-1">
              <label className="block text-sm font-semibold text-[#1f2937]/80 mb-2">Предпросмотр фото</label>
